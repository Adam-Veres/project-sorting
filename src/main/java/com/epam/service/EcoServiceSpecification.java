...
public class EcoServiceSpecification {
  ...
  public static Specification<EcoService> hasAreaLatitude(
      final BigDecimal startLatitude, final BigDecimal stopLatitude) {
    return (root, cq, cb) ->
        cb.between(
            root.get(EcoService_.coordinate).get(Coordinate_.latitude),
            startLatitude,
            stopLatitude);
  }

  public static Specification<EcoService> hasAreaLongitude(
      final BigDecimal startLongitude, final BigDecimal stopLongitude) {
    return (root, cq, cb) ->
        cb.between(
            root.get(EcoService_.coordinate).get(Coordinate_.longitude),
            startLongitude,
            stopLongitude);
  }
  ...
  public static Specification<EcoService> hasPaymentCondition(
